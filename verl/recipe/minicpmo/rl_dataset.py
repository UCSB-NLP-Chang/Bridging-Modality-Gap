...
def process_minicpmo_data(
    ...
    if image_key in row_dict and len(row_dict[image_key]) == 1:
        multi_modal_data = {}
...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_contrastive_kl = config.get("use_contrastive_kl", False)
        self.contrastive_key = config.get("contrastive_key", "contrastive_prompt")
    ...
    def _build_messages(self, example: dict, is_contrastive: bool = False):
        """
        Align with the first snippet: support contrastive messages.
        """
        key = self.contrastive_key if is_contrastive else self.prompt_key
        return example.pop(key)
    ...
    def __getitem__(self, item):
        ...
        messages = self._build_messages(row_dict, is_contrastive=False)
        model_inputs = {}
        ...
        # =============================
        # Contrastive KL branch (added)
        # =============================
        if self.use_contrastive_kl:
            contrastive_row_dict: dict = self.dataframe[item]
            contrastive_messages = self._build_messages(contrastive_row_dict, is_contrastive=True)

            if self.processor is not None and (self.image_key in contrastive_row_dict):
                # Try MiniCPM-o pipeline; fall back to text-only if not supported
                try:
                    c_model_inputs, _, c_raw_prompt = process_minicpmo_data(
                        contrastive_row_dict,
                    ...
                    c_input_ids = c_model_inputs.pop("input_ids")
                    c_attention_mask = c_model_inputs.pop("attention_mask")
                    c_position_ids = c_model_inputs.pop("position_ids")
                except NotImplementedError:
                    # fallback to pure text
                    c_raw_prompt = self.tokenizer.apply_chat_template(
                        contrastive_messages, add_generation_prompt=True, tokenize=False
                    )
                    c_tok = self.tokenizer(c_raw_prompt, return_tensors="pt", add_special_tokens=False)
                    c_input_ids = c_tok.pop("input_ids")
                    c_attention_mask = c_tok.pop("attention_mask")
                    c_position_ids = compute_position_id_with_mask(c_attention_mask)
            else:
                # text-only path
                c_raw_prompt = self.tokenizer.apply_chat_template(
                    contrastive_messages, add_generation_prompt=True, tokenize=False
            ...
            # assign contrastive tensors
            row_dict["contrastive_input_ids"] = c_input_ids
            row_dict["contrastive_attention_mask"] = c_attention_mask
            row_dict["contrastive_position_ids"] = c_position_ids
